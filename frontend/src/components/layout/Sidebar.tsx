...
import {
  ...
  Building2, ChevronDown, CheckCircle2, Landmark, Users, FolderOpen,
} from 'lucide-react'
...
export function Sidebar() {
  const { sidebarCollapsed, toggleSidebar, companies, selectedRealmId, setCompanies, setSelectedRealmId } = useAppStore()
  ...
  useEffect(() => {
    api.integrations.qboCompanies()
      ...
      .catch(() => {
        // Fallback: show known companies even if API fails
        const fallback = [
          { realm_id: '9341454010854556', company_name: 'Allapattah CDC', connected: true, token_expired: false, expires_at: '' },
          { realm_id: '9341454429254087', company_name: '3010 LLC', connected: true, token_expired: false, expires_at: '' },
        ]
        setCompanies(fallback)
        if (!selectedRealmId) setSelectedRealmId(fallback[0].realm_id)
      })
  }, [])
  ...
  const selectedCompany = companies.find(c => c.realm_id === selectedRealmId)
  ...
  return (
    ...
    >
      ...
      {!sidebarCollapsed && companies.length > 0 && (
        <div className="px-2 pt-3 pb-1 relative" ref={dropdownRef}>
          <button
            onClick={() => setCompanyOpen(v => !v)}
            className="w-full flex items-center gap-2 rounded-lg bg-surface-800 border border-surface-700 px-2.5 py-2 text-left hover:bg-surface-700 transition-colors"
          >
            <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-emerald-600/20">
              <Building2 size={11} className="text-emerald-400" />
            </div>
            <span className="flex-1 truncate text-[12px] font-medium text-surface-200">
              {selectedCompany?.company_name ?? 'Select company'}
            </span>
            <ChevronDown size={11} className="text-surface-500 shrink-0" />
          </button>

          {companyOpen && (
            <div className="absolute left-2 right-2 top-full z-50 mt-1 rounded-xl border border-surface-700 bg-surface-800 py-1 shadow-xl animate-fade-in">
              {companies.map(c => (
                <button
                  key={c.realm_id}
                  onClick={() => { setSelectedRealmId(c.realm_id); setCompanyOpen(false) }}
                  className={cn(
                    'w-full flex items-center gap-2.5 px-3 py-2 text-left text-[12px] transition-colors',
                    c.realm_id === selectedRealmId
                ...
                >
                  <CheckCircle2
                    ...
                    className={cn(
                      'shrink-0',
                      c.token_expired ? 'text-amber-400' : 'text-emerald-400',
                    )}
                  />
                  <span className="flex-1 truncate font-medium">{c.company_name}</span>
                  {c.realm_id === selectedRealmId && (
      ...
      {sidebarCollapsed && selectedCompany && (
        <div className="flex justify-center pt-3 pb-1">
          <div
            ...
            className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-600/20 border border-emerald-700/40"
          >
